...
/**
 * Hard-rule detection for document listing requests
 * e.g., "list my documents", "show my files", "name all the documents I uploaded"
 */
function isDocumentListingIntent(message = '') {
  const m = (message || '').toLowerCase();
  // Common verbs + nouns around docs/files/uploads/history
  const verbs = /(list|show|name|what|which|display|give|see|view|all)\b/;
  const nouns = /(doc|docs|document|documents|file|files|upload|uploads|history)/;
  const possessive = /(my|me|i\s+uploaded|i\s+have\s+uploaded|i\s+uploaded|that\s+i\s+uploaded)/;
  // Strong patterns like "list my documents" or "name all the documents I uploaded"
  if (verbs.test(m) && nouns.test(m) && (possessive.test(m) || m.includes('i uploaded') || m.includes('uploaded'))) return true;
  // Additional concise patterns
  if (/\b(list|show)\b[^\n]*\b(my\s+)?(docs|documents|files|uploads)\b/.test(m)) return true;
  return false;
}
...
function analyzeIntent(message) {
  ...
  // Prefer document-agent if it's clearly a document listing request
  if (isDocumentListingIntent(message_lower)) {
  ...
  message_lower.includes('pdf') ||
  message_lower.includes('lab')) {
    return 'document-agent';
  }
...
exports.handler = async (event) => {
  try {
    ...
    if (requestedAgent && requestedAgent !== 'auto') {
      ...
      // Hard-rule: if it's clearly a document listing request, route directly to document-agent
      if (isDocumentListingIntent(message)) {
        agentType = 'document-agent';
        console.log('Hard-routed to document-agent for document listing intent');
      } else {
        try {
    ...
    if (result.body) {
      ...
      responseBody.routingDecision = (requestedAgent && requestedAgent !== 'auto') ? 'explicit' : 'inferred';
